...
class LoadQt(QMainWindow):
    def __init__(self):
        ...
        loadUi("opencv_UI\demo.ui", self)
        self.setWindowIcon(QtGui.QIcon("opencv_UI\python-icon.png"))
    ...
    def displayImage(self, window=1):
        ...
        label = self.imgLabel if window == 1 else self.imgLabel2

        # Calculate aspect ratio
        aspect_ratio = self.image.shape[1] / self.image.shape[0]

        # Set a fixed width or height based on the aspect ratio
        fixed_width = 450  # You can adjust this value as needed
        fixed_height = int(fixed_width / aspect_ratio)

        # Create a scaled pixmap
        pixmap = QPixmap.fromImage(img)
        scaled_pixmap = pixmap.scaled(fixed_width, fixed_height, QtCore.Qt.KeepAspectRatio)

        label.setPixmap(scaled_pixmap)

        # Set alignment to center the image within the label
        label.setAlignment(QtCore.Qt.AlignCenter)

    def open_img(self):
        fname, filter = QFileDialog.getOpenFileName(self, 'Open File', 'C:\\Users\\Argha Kamal Samanta', "Image Files (*)")
        if fname:
    ...
    def AboutMessage2(self):
        QMessageBox.about(self, "About Author", "Author: Argha Kamal Samanta \n\n")
...
########################################Cartooning of an Image##########################################################
    def cartoon(self):
        ...
        h, w, _ = img_edge.shape
        img_color = cv2.resize(img_color, (w, h))

        print(f"shape of img: {self.image.shape}")
        print(f"shape of img_edge: {img_edge.shape}")
        print(f"shape of img_color: {img_color.shape}")
